...
check_network() {
    ...
    if curl -s --max-time 5 https://registry-1.docker.io >/dev/null 2>&1; then
        success "Conexión a Docker Hub OK"
    else
...
check_docker_script() {
    ...
    if [[ -x "$DOCKER_SCRIPT" ]]; then
        ...
        # Verificar que puede detectar Docker Compose
        if bash "$DOCKER_SCRIPT" --help >/dev/null 2>&1; then
            success "docker.sh funciona correctamente"
        else
            error "docker.sh tiene problemas"
            return 1
        fi
    else
...
main() {
    echo
    info "Verificación del entorno para Microservicios"
    echo "==============================================="
    ...
    if [ "$checks_passed" -eq "$total_checks" ]; then
        ...
        info "  • Desplegar:     ./deploy.sh"
        info "  • Monitorear:     ./monitor.sh"
        info "  • Limpiar:        ./cleanup.sh"
        info "  • Docker alias:   ms (después de instalar)"
        echo
        info "Para instalar alias persistente: ./docker.sh install-alias"
    else
