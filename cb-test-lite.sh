...
# Extrae "state":"closed|open|half-open" del JSON de manera portable (sin jq/python)
extract_state() {
  # lee de STDIN
  tr -d '\r\n ' | grep -o '"state":"[^"]*"' | head -1 | sed 's/.*"state":"\([^"]*\)".*/\1/' || true
}
...
discover_cb_url() {
  ...
  for p in "${PORTS[@]}"; do
    for path in "${CB_PATHS[@]}"; do
      ...
      if out="$(curl -fs --max-time 2 "$url" 2>/dev/null || true)"; then
